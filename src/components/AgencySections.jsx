...
const AgencyCard = ({
  ...
  className = "",
  highlight = false,
  ...restProps // Renamed to restProps
}) => {
  ...
  // Construct the full className string here for clarity and to potentially avoid parsing issues
  ...
  return (
    <motion.div
      ...
      // Pass the constructed class name directly
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {[
            ...
            <AgencyCard key={pkg.title} delay={idx * 0.1} highlight={pkg.highlight} className="min-h-[260px] sm:min-h-[280px] md:min-h-[300px] flex flex-col justify-between items-center">
              {/* REMOVED: pkg.highlight conditional div */}
              <h3
                className="text-lg sm:text-xl md:text-2xl font-bold mb-2 text-black"
                style={{ transform: "translateZ(20px)" }}
              >
    ...
    <section className="w-full py-16 px-4 sm:py-20 sm:px-6 md:px-8 bg-transparent" id="blog">
      <div className="max-w-6xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
          {[
            {
              title: "How to Choose the Right Web Agency",
              desc: "Tips for finding a partner that fits your business goals and budget.",
            },
            {
              title: "Web Design Trends 2025",
              desc: "A look at the latest design and technology trends shaping the web.",
            },
          ].map((blog, idx) => (
            <AgencyCard key={blog.title} delay={idx * 0.1} className="min-h-[180px] sm:min-h-[200px] md:min-h-[220px] flex justify-between items-center">
              <div className="flex flex-col items-center text-center">
                <h3
                  className="text-lg sm:text-xl md:text-2xl font-bold mb-2 text-black"
                  style={{ transform: "translateZ(20px)" }}
